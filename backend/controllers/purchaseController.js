...
console.log('[BOOT][purchaseController] PAYPAL:', hasPaypalCreds ? 'OK' : 'MISSING', '| SMTP:', hasEmailCreds ? 'OK' : 'MISSING');
...
async function fulfillCourseAccess({ userId, email, name, courseSlug, courseTitle }) {
  ...
  await CourseAccess.updateOne(
    { userId, courseSlug },
    { $setOnInsert: { userId, courseSlug, provider: 'paypal', grantedBy: 'paypal', grantedAt: new Date() } },
    { upsert: true }
  );
...
export const createOrder = async (req, res) => {
  try {
    ...
    // Si tenés middleware de auth, podés validar req.user
    // (no es estrictamente obligatorio para crear una orden)
    ...
    request.requestBody({
      ...
      purchase_units: [
        {
          amount: { currency_code: currency, value: amountValue.toFixed(2) },
          description,
          custom_id: customId,
        },
      ],
...
export const captureOrder = async (req, res) => {
  try {
    ...
    const { orderID, courseSlug: courseSlugBody, email: emailFromBody, name: nameFromBody } = req.body || {};
    ...
    console.error('[PayPal] captureOrder error:', error?.message || error);
